...
class MILLoss(nn.Module):
    ...
    def forward(self, y_true, y_pred):
        ...
        n_seg = 16    # Because we have 32 segments per video.
        nvid = 8
        n_exp = nvid / 2
        Num_d = 32 * nvid

        sub_max = torch.ones_like(
            y_pred)    # sub_max represents the highest scoring instants in bags (videos).
        sub_sum_labels = torch.ones_like(
            y_true
        )    # It is used to sum the labels in order to distinguish between normal and abnormal videos.
        sub_sum_l1 = torch.ones_like(
            y_true)    # For holding the concatenation of summation of scores in the bag.
        ...
        #  NOTE: F_labels contains integer 32 for normal video and 0 for abnormal videos. This because of labeling done at the end of "load_dataset_Train_batch"
